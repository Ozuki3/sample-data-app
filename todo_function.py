...
def edit_ppt(Property,Start_Date,End_Date,property,start,end):
	c.execute("UPDATE taskstable SET Property=? ,Start_Date=?, End_Date=? WHERE Property=? and Start_Date=? and End_Dateand=? ",(Property,Start_Date,End_Date,property,start,end))
	conn.commit()
	data = c.fetchall()
	return data
